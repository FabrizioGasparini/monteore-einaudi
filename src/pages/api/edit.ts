...
const handle = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const { id, name, aula, duration, desc, date, startTime, endTime, maxNumber } = req.body;
    ...
    if (duration < 1 || duration > 5) return res.status(400).json({ message: "La durata deve essere compresa tra 1 e 5 ore" });
